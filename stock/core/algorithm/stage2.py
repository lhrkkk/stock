...
def Ek(self):
    '''
    为dataframe自身计算能量, 势能越小, 动能部分就越大, 越低越好
    底部是弹性势能, 顶部是重力势能, 中间段尤其是刚反弹的时候动能最大, 逐渐减小.
    :param data: 输入的dataframe
    :return: 返回计算后的data
    '''
    data['buy_score']=0
    if (data['dmacdhist']>0)&(data['macdhist']>0) & ((data['macdhist']-2*data['dmacdhist'])<0 ) :
        data['buy_score']+=100 # 确切直接金叉
    if (data['dmacdhist']>0)&(data['macdhist']<0)& ((data['macdhist']+2*data['dmacdhist'])>0 ) :
        data['buy_score']+=100 # 斜率大于0且向上2个周期触碰0点

    if ((data['macdhist']-data['dmacdhist'])<0 ) :
        data['buy_score']+=100
    if (data['macd']<0) & (data['macdsignal']<0) :
        data['buy_score']+=100
    if (( data['dma61']> data['dma131']) |(data['ma61']>data['ma131'])) :
        data['buy_score']+=100
    if (data['ddma5']>=0)&(data['dma5']>0) :
        data['buy_score']+=100
...
def p(self):
    '''
    为dataframe自身计算动量.
    动量和动能差不多, 但是有方向, 底部弹性势能产生的方向是向上, 顶部重力势能转化来的方向向下
    :param data: 输入的dataframe
    :return: 返回计算后的data
    '''
...
@as_method_of(DataFrame):
def av(self):
    dmacd=self['dmacdhist']
    macd=self['macdhist']
    dma61=self['dma61']
    dma131=self['dma131']
    volume=self['volume']
    ddma5=self['dma5'].diff()
...
# if (self['dmacdhist']>0)&(self['macdhist']>0) & ((self['macdhist']-2*self['dmacdhist'])<0 ) :
    #     self['av']+=100 # 确切直接金叉
    # if (self['dmacdhist']>0)&(self['macdhist']<0)& ((self['macdhist']+2*self['dmacdhist'])>0 ) :
    #     self['av']+=100 # 斜率大于0且向上2个周期触碰0点
    #
    # if ((self['macdhist']-self['dmacdhist'])<0 ) :
    #     self['av']+=100
    # if (self['macd']<0) & (self['macdsignal']<0) :
    #     self['av']+=100
    # if (( self['dma61']> self['dma131']) |(self['ma61']>self['ma131'])) :
    #     self['av']+=100 # 均线走势好
    # if (self['ddma5']>=0)&(self['dma5']>0) :
    #     self['buy_score']+=100 #极小值
